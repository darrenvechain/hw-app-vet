...
export default class VET {
    ...
    public async getAccount(
        ...
        const acc: Account = {
            ...
            address: '0x' +
                response.slice(1 + publicKeyLength + 1, 1 + publicKeyLength + 1 + addressLength)
                    .toString('ascii')
                    .toLowerCase()
        }
        if (chainCode) {
    ...
    public async signTransaction(path: string, rawTx: Buffer) {
    ...
    private async sign(path: string, raw: Buffer, isTransaction: boolean, cla: number, ins: number) {
        const buffers = splitRaw(path, raw, isTransaction)
        const responses = [] as Buffer[]
        for (let i = 0; i < buffers.length; i++) {
            const data = buffers[i]
            responses.push(await this.transport.send(cla, ins, i === 0 ? 0x00 : 0x80, 0x00, data, [StatusCodes.OK]))
        }
